...
public abstract class FileHostingServiceBase<TFile> : IFileHostingService<TFile>
    where TFile : IStorageFile
{
    ...
    public abstract Task<IEnumerable<TFile>> AddFilesToDb(
        IReadOnlyList<UploadFileProposal> files,
        CancellationToken cancellationToken = default
    );
    public abstract Task<Dictionary<string, TFile>> GetFilesByHash(
        string userId,
        IReadOnlyCollection<string> hash
    );
    ...
    /// <returns></returns>
    public virtual async Task<IEnumerable<TFile>> UploadFiles(
        string? UserId,
    ...
    )
    {
        ...
        foreach (var (file, index) in files.Select((item, index) => (item, index)))
        {
            ...
            if (file.Length > 0)
            {
                ...
                var hash = (
                    await memoryStream.GetHashMd5(cancellationToken: cancellationToken)
                ).BytesToBase64String();
                ...
                {
                    var objectName =
                        _settings.BucketSubPath + GetFileSubPath(UserId, name, contentType);
                    ...
                    var mediaLink =
                        $"https://firebasestorage.googleapis.com/v0/b/{_settings.BucketName}/o/{Uri.EscapeDataString(objectName)}?alt=media";
                    toAddList.Add(
                        new UploadFileProposal()
                        {
    ...
    public virtual async Task<Google.Apis.Storage.v1.Data.Object?> GetBucketObject(
        IStorageFile file,
        CancellationToken cancellationToken = default
    )
    {
        ...
        {
            var objectName =
                _settings.BucketSubPath
                + GetFileSubPath(file.UploaderId, file.FileName, file.ContentType);
            var res = await _storageClient.GetObjectAsync(
                _settings.BucketName,
                objectName,
                cancellationToken: cancellationToken
            );
            if (res == null || res.TimeDeletedDateTimeOffset != null)
            {
                return null;
            }
            return res;
